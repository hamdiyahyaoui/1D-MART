...
function [dist_arr] = CP_MinDist_K(str11, str_list1)
...
if exist('SAXT85K.xlsx', 'file')==0 
    tic;
    dataTTT = xlsread('SAXT85.xlsx');
    [~, TrendT, ~]=xlsread('SDT85K.xlsx');
    delete('SAXT85K.xlsx'); delete('SEQT85K.xlsx'); delete('BETAT85K.xlsx');
    dataTT=dataTTT(:, 1:end);
    [No_SeqOT, L]=size(dataTT);
    Terms=floor(L/TermSize);
    for SEQO=1:No_SeqOT
        X=Distance_Computation_K(dataTT(SEQO,:), TrendT(SEQO,:),TermSize);
        AT(SEQO,:)=X.m;  AAT(SEQO,:)=X.d;  AAAT(SEQO,:)=X.sq;  AAAAT(SEQO,:)=X.t;
    end
     xlswrite('SAXT85K.xlsx',AT);   xlswrite('SEQT85K.xlsx',AAT);  xlswrite('BETAT85K.xlsx',AAAT);  
%      idx=cellfun(@isstr,AAAAT);
%      AAAAT(idx)=strcat('''', AAAAT(idx));
%      xlswrite('SDT85K.xlsx',AAAAT);
     SDTT.m= xlsread('SAXT85K.xlsx');
     SDTT.d= xlsread('SEQT85K.xlsx');
     SDTT.sq= xlsread('BETAT85K.xlsx');
     [~, SDTT.t, ~]= xlsread('SDT85K.xlsx');
     toc;
end


if exist('SAXS85K.xlsx', 'file')==0
    dataOOO = xlsread('SAXS85.xlsx'); 
    [~, TrendO, ~]=xlsread('SDS85K.xlsx');
    delete('SAXS85K.xlsx'); delete('SEQS85K.xlsx'); delete('BETAS85K.xlsx'); 
    dataOO=dataOOO(:,1:end);
    [No_SeqOS, ~]=size(dataOO);
    for SEQO=1:No_SeqOS
        Y=Distance_Computation_K(dataOO(SEQO,:), TrendO(SEQO,:), TermSize);
        AS(SEQO,:)=Y.m;    AAS(SEQO,:)=Y.d;   AAAS(SEQO,:)=Y.sq;   AAAAS(SEQO,:)=Y.t;
    end
     xlswrite('SAXS85K.xlsx', AS);  xlswrite('SEQS85K.xlsx',AAS); xlswrite('BETAS85K.xlsx',AAAS); 
%      idx=cellfun(@isstr, AAAAS);
%      AAAAS(idx)=strcat('''', AAAAS(idx));
%      xlswrite('SDS85K.xlsx',AAAAS);
     str1111.m= xlsread('SAXS85K.xlsx');
     str1111.d= xlsread('SEQS85K.xlsx');
     str1111.sq= xlsread('BETAS85K.xlsx');
     [~, str1111.t, ~]= xlsread('SDS85K.xlsx');  
end

dataO=xlsread(TestFile);
for SEQO=1:No_SeqOS
    if dataO(SEQO,:) == str11
...
for j=1:s_row
      ...
      Rho_SQ(Terms)=mean(Corr_SQ(segment(Terms):L));
